...
def _clip_Conv1D_grad(layer: pytorch_utils.Conv1D, A: torch.Tensor, B: torch.Tensor, C) -> None:
    try:
        grad_weight = torch.einsum('b...,b->...',layer.weight.grad_sample,C)
        del layer.weight.grad_sample
    except:
...
_supported_layers_norm_sample_AND_clipping = {
    ...
    pytorch_utils.Conv1D: (_compute_Conv1D_grad_sample, _clip_Conv1D_grad),# Conv1D's weight is transposed to nn.Linear's, but this does not matter for the norm
    models.t5.modeling_t5.T5LayerNorm: (_compute_t5_layer_norm_grad_sample, _clip_t5_layer_norm_grad),
}
