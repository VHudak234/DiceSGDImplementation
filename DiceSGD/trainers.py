...
def EFSGD(model, train_dl, test_dl, batch, sample_size, minibatch, epoch, C, C2, device, lr, method, logger):
    if method == 'sgd':
        optimizer=torch.optim.SGD(model.parameters(), lr=lr) 
    elif method == 'adam':
        optimizer=torch.optim.Adam(model.parameters(), lr = lr)
...
def DiceSGD(model, train_dl, test_dl, batch, sample_size, minibatch, epoch, C, C2, device, lr, method, logger):
    if method == 'sgd':
        optimizer=torch.optim.SGD(model.parameters(), lr=lr) 
    elif method == 'adam':
        optimizer=torch.optim.Adam(model.parameters(), lr = lr)
